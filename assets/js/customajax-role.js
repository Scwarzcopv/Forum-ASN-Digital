...
function all() {
    ...
    $.ajax({
        ...
        url: base_url+"Admin/all", // get the route value
        success: function(response) {
            ...
            if (response.length) {
                ...
                $.each(response, function(key, value) {
                    ...
                    html += '<a class="btn btn-warning text-light" href="'+base_url+'admin/roleaccess/'+value.id+'" id="btnAccessMenu" name="btnAccessMenu"><i class="align-middle fa-solid fa-universal-access"></i></a>';
                    html += '<buttton class="btn btn-primary mx-1" data-bs-target="#modalEdit" data-bs-toggle="modal" data-id='+
                        value.id+' id="btnEditMenu" name="btnEditMenu"><i class="align-middle fa-solid fa-pen"></i></buttton>';
                    html += '<button class="btn btn-danger" data-id='+
                        value.id+' data-val='+value.role+' id="btnHapusMenu" name="btnHapusMenu"><i class="align-middle fa-solid fa-trash"></i></button>';
                    html += '</td>';
                    html += '</tr>';
                });
            } else {
...
//---- TIPPY JS ----
function tippyJs(target, content) {
    tippy.delegate('.table-responsive', {
        target: target,
        content: content,
...
$(document).ready(function () {
    ...
    //Tippy
    tippyJs('#btnAccessMenu', 'Edit akses user');
    tippyJs('#btnEditMenu', 'Edit menu user');
    tippyJs('#btnHapusMenu', 'Hapus menu user');
});